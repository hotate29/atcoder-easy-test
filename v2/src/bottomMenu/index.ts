...
async function init(): Promise<BottomMenu> {
  ...
  const bottomMenuKey = bottomMenu.querySelector(
    "#bottom-menu-key",
  ) as HTMLButtonElement;
  const bottomMenuTabs = bottomMenu.querySelector(
    "#bottom-menu-tabs",
  ) as HTMLUListElement;
  const bottomMenuContents = bottomMenu.querySelector(
  ...
  const menuController: BottomMenu = {
    ...
    addTab(
      ...
      console.log(
        `AtCoder Easy Test: addTab: ${tabLabel} (${tabId})`,
        paneContent,
      );
