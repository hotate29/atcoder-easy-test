...
(async () => {
  ...
  const atCoderEasyTest = {
    ...
    runTest(
    ...
    ): [Promise<Result>, Promise<BottomMenuTab>] {
      this.disableButtons();
      ...
      const pTab = pBottomMenu.then((bottomMenu) =>
        bottomMenu.addTab(
          ...
          `#${++this.runCount} ${title}`,
          content.element,
          { active: true, closeButton: true },
        ),
      );

      const pResult = codeRunner.run(
        language,
        sourceCode,
        input,
        output,
  ...
  // place "Easy Test" tab
  {
    ...
    const E = <T extends HTMLElement>(id: string) =>
      root.querySelector<T>(`#atcoder-easy-test-${id}`);
    ...
    // 言語選択関係
    {
      ...
      async function setLanguage() {
        ...
        try {
          ...
          console.log(
            `AtCoder Easy Test: language = ${langs[1]} (${langs[0]})`,
          );
    ...
    function runTest(
      ...
      return atCoderEasyTest.runTest(
        title,
        eLanguage.value,
        site.sourceCode,
        input,
    ...
    function runAllCases(testcases: TestCase[]): Promise<Result[]> {
      ...
      const pairs = testcases.map((testcase) =>
        runTest(testcase.title, testcase.input, testcase.output, {
          runGroupId,
        }),
      );
