...
const runners: { [runnerId: string]: CodeRunner } = {
  ...
  "C++ GCC 10.2.0 + Boost 1.73.0 + ACL Wandbox": new WandboxCppRunner(
    "gcc-10.2.0",
    "C++ (GCC 10.2.0) + ACL",
    {
      "compiler-option-raw":
  ...
  "C++ GCC 9.3.0 + Boost 1.73.0 + ACL Wandbox": new WandboxCppRunner(
    "gcc-9.3.0",
    "C++ (GCC 9.3.0) + ACL",
    {
      "compiler-option-raw":
  ...
  "C++ Clang 10.0.1 + ACL Wandbox": new WandboxCppRunner(
    "clang-10.0.1",
    "C++ (Clang 10.0.1) + ACL",
    {
      "compiler-option-raw":
  ...
  "C# .NET Core 3.1.407 Wandbox": new WandboxRunner(
    "dotnetcore-3.1.407",
    "C# (.NET Core 3.1.407)",
  ),
  "C# Mono-mcs 6.12.0.122 Wandbox": new WandboxRunner(
    "mono-6.12.0.122",
    "C# (Mono-mcs 6.12.0.122)",
  ),
  "Clojure paiza.io": new PaizaIORunner("clojure", "Clojure"),
  "Crystal 0.36.1 Wandbox": new WandboxRunner(
    "crystal-0.36.1",
    "Crystal (0.36.1)",
  ),
  ...
  "Erlang 22.3.4.16": new WandboxRunner(
    "erlang-22.3.4.16",
    "Erlang (22.3.4.16)",
  ),
  ...
  "Haskell GHC 8.8.4 Wandbox": new WandboxRunner(
    "ghc-8.8.4",
    "Haskell (GHC 8.8.4)",
  ),
  "Haskell paiza.io": new PaizaIORunner("haskell", "Haskell"),
  "Java openjdk-jdk-15.0.3+2 Wandbox": new WandboxRunner(
    "openjdk-jdk-15.0.3+2",
    "Java (JDK 15.0.3+2)",
  ),
  "Java openjdk-jdk-14.0.2+12 Wandbox": new WandboxRunner(
    "openjdk-jdk-14.0.2+12",
    "Java (JDK 14.0.2+12)",
  ),
  "JavaScript paiza.io": new PaizaIORunner("javascript", "JavaScript"),
  "JavaScript Node.js 12.22.1 Wandbox": new WandboxRunner(
    "nodejs-12.22.1",
    "JavaScript (Node.js 12.22.1)",
  ),
  ...
  "Lua LuaJIT 2.0.5 Wandbox": new WandboxRunner(
    "luajit-2.0.5",
    "Lua (LuaJIT 2.0.5)",
  ),
  ...
  "Pascal FPC 3.0.4 Wandbox": new WandboxRunner(
    "fpc-3.0.4",
    "Pascal (FPC 3.0.4)",
  ),
  ...
  "Python PyPy 7.3.4 Wandbox": new WandboxRunner(
    "pypy-2.7-v7.3.4",
    "PyPy2 (7.3.4)",
  ),
  "Python3 PyPy3 7.3.4 Wandbox": new WandboxRunner(
    "pypy-3.7-v7.3.4",
    "PyPy3 (7.3.4)",
  ),
  ...
  "TypeScript typescript-3.9.9 nodejs 14.16.1 Wandbox": new WandboxRunner(
    "typescript-3.9.9 nodejs 14.16.1",
    "TypeScript (3.9.9)",
  ),
  "Text local": new CustomRunner(
    "Text",
  ...
  "COBOL Fixed OpenCOBOL 1.1.0 AtCoder": new AtCoderRunner(
    "4060",
    "COBOL - Fixed (OpenCOBOL 1.1.0)",
  ),
  "COBOL Free OpenCOBOL 1.1.0 AtCoder": new AtCoderRunner(
    "4061",
...
config.registerCount(
  "codeRunner.maxRetry",
  3,
  "Max count of retry when IE (Internal Error)",
);
...
export default {
  ...
  async run(
    ...
    options: Options = { trim: true, split: true },
  ): Promise<Result> {
    ...
    if (sourceCode.length > 0)
      pSite.then((site) => codeSaver.save(site.taskURI, sourceCode));

    // 実行
    const maxRetry = config.get("codeRunner.maxRetry", 3);
    for (let retry = 0; retry < maxRetry; retry++) {
      try {
        const result = await runners[runnerId].test(
          sourceCode,
          input,
          expectedOutput,
          options,
        ...
        if (result.status == "IE") {
          ...
          const runnerIds = Object.keys(runners).filter(
            (runnerId) => runnerId.split(" ")[0] == lang,
          );
