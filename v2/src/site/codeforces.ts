...
config.registerFlag(
  "site.codeforces.showEditor",
  true,
  "Show Editor in Codeforces Problem Page",
);
...
async function init() {
  ...
  const eLang = doc.querySelector<HTMLSelectElement>(
...
.atcoder-easy-test-btn-run-case {
  ...
  let _sourceCode = "";
  const eFile =
    doc.querySelector<HTMLFormElement>(".submitForm").elements["sourceFile"];
  ...
  const waitCfFastSubmit = setInterval(() => {
    if (document.getElementById("editor")) {
      ...
      // 言語セレクトを同期させる
      const eLang2 = doc.querySelector<HTMLSelectElement>(
        ".submit-form select[name='programTypeId']",
      );
      if (eLang2) {
        eLang.addEventListener("change", () => {
  ...
  return {
    ...
    set sourceCode(sourceCode: string) {
      ...
      container.items.add(
        new File([sourceCode], "prog.txt", { type: "text/plain" }),
      );
      const eFile =
        doc.querySelector<HTMLFormElement>(".submitForm").elements[
          "sourceFile"
        ];
    ...
    get testCases(): TestCase[] {
      ...
      for (const eSampleTest of doc.querySelectorAll(".sample-test")) {
        ...
        const anchors = eSampleTest.querySelectorAll(
          ".input .title .input-output-copier",
        );
